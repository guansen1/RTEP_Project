...
TelegramListener::TelegramListener(const std::string& token, const std::string& chatId)
    : token(token), chatId(chatId), detectionMode(false), running(false) {}
...
void TelegramListener::run() {
    ...
    while (running) {
        ...
        // 检查是否有 "start detection" 或 "stop detection" 命令
