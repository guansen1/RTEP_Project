...
bool GPIO::configGPIO(int pin_number, int config_num) {
    ...
    if (!line) {
        std::cerr << "cant get GPIO pin " << pin_number << std::endl;
        return false;
    }
    ...
    switch (config_num) {
        ...
        default:
            std::cerr << "error: unknown GPIO pin " << config_num << std::endl;
            gpiod_line_release(line);
            return false;
    }

    if (request_status < 0) {
        std::cerr << "cant congfig GPIO " << pin_number << "mode: " << config_num << "）\n";
        return false;
    }
...
int GPIO::readGPIO(int pin_number) {
    if (gpio_pins.find(pin_number) == gpio_pins.end()) {
        std::cerr << "GPIO pin " << pin_number << " uninitialized!" << std::endl;
        return -1;
    }
...
bool GPIO::writeGPIO(int pin_number, int value) {
    if (gpio_pins.find(pin_number) == gpio_pins.end()) {
        std::cerr << "GPIO " << pin_number << " uninitialized" << std::endl;
        return false;
    }
    if (gpio_config[pin_number] != 1) {
        std::cerr << "GPIO " << pin_number << " not output mode, cant write in!" << std::endl;
        return false;
    }
...
bool GPIO::readEvent(int pin_number, gpiod_line_event& event) {
    ...
    if (gpiod_line_event_read(gpio_pins[pin_number], &event) < 0) {
        std::cerr << "read GPIO " << pin_number << " event fail\n";
        return false;
    }
    return true;
}
